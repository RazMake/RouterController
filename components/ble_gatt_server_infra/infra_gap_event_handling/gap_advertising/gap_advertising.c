...
{
    // Then start advertising with the current settings, but only if all are successfully set:
    if (advertising_configured_bits != 0)
    {
        return;
    }

    ESP_LOGI(COMPONENT_TAG, "Starting BLE advertising");
...
void on_advertising_data_set(esp_bt_status_t setStatus)
{
    ...
    {
        ESP_LOGE(COMPONENT_TAG, "Setting advertising data failed: %d", setStatus);
        abort();
    }
    else
    {
...
void on_scan_response_set(esp_bt_status_t setStatus)
{
    if (setStatus != ESP_BT_STATUS_SUCCESS)
    {
        ESP_LOGE(COMPONENT_TAG, "Setting scan response data failed: %d", setStatus);
        abort();
    }
    else
    {
...
{
    // First set the flags, to be sure we get to set them correctly before the infra gets a chance to fire the 'completed' event for any of these:
    advertising_configured_bits |= adv_data != NULL ? advertising_data_configured_bit : 0;
    advertising_configured_bits |= scan_rsp_data != NULL ? scan_response_configured_bit : 0;

    if (adv_data != NULL)
    {
        ESP_LOGI(COMPONENT_TAG, "Configure advertising data");
        ESP_ERROR_CHECK_WITHOUT_ABORT(esp_ble_gap_config_adv_data(adv_data));
    }

    if (scan_rsp_data != NULL)
    {
        ESP_LOGI(COMPONENT_TAG, "Configuring scan response data");
