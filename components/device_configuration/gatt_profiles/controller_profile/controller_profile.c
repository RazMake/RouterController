...
#include "esp_log.h"
#define COMPONENT_TAG "ROUTER_CONTROLLER_PROFILE"
...
static void router_controller_profile_handler(esp_gatts_cb_event_t event, esp_ble_gatts_cb_param_t *param)
{
    switch(event)
    {
        ...
        // This event is fired whenever the central device attempts to read a characteristic's value.
        case ESP_GATTS_READ_EVT: {
            ESP_LOGI(COMPONENT_TAG, "Read, conn_id %d, trans_id %" PRIu32 ", handle %d", param->read.conn_id, param->read.trans_id, param->read.handle);
            // esp_gatt_rsp_t rsp;
    ...
    case ESP_GATTS_DISCONNECT_EVT:
        ESP_LOGI(GATTS_TAG, "ESP_GATTS_DISCONNECT_EVT, disconnect reason %d", param->disconnect.reason);
